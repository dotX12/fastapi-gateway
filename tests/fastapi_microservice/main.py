...
async def query_and_body_path(
    query_int: int, query_str: str, path: int, foo_model: ExampleModel
):
    return {
        "query_int": query_int,
        "query_str": query_str,
...
async def form_and_upload_file(
    username: str = Form(...), password: str = Form(...), file: UploadFile = File(...)
):
    return {
        "username": username,
        "pwd": password,
...
@app.get(path="/v1/check_dependency_header")
